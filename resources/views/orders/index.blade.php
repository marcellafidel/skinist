...
<body class="bg-sky-50 text-gray-800">
    ...
    <main class="max-w-4xl mx-auto px-4 py-8">
        ...
        {{-- INFO REKENING --}}
        <div class="bg-sky-100 rounded-2xl p-5 mb-6 border border-sky-200">
            <p class="font-bold text-sky-700 mb-2">📋 Info Pembayaran</p>
            <p class="text-sm text-sky-600">Transfer ke rekening berikut:</p>
            <p class="text-sm text-sky-700 font-semibold mt-1">BCA — 1234567890 a/n Skinist Store</p>
            <p class="text-xs text-sky-500 mt-1">Setelah transfer, upload bukti pembayaran di pesanan kamu.</p>
        </div>
            ...
            @if($order->status === 'pending')
                @if($order->payment_proof)
                    <div class="mt-4 p-3 bg-yellow-50 rounded-xl border border-yellow-200">
                        <p class="text-xs text-yellow-600 font-semibold">✅ Bukti pembayaran sudah diupload, menunggu konfirmasi admin.</p>
                        <img src="{{ asset('storage/' . $order->payment_proof) }}" class="mt-2 h-24 rounded-lg object-cover">
                    </div>
                @else
                    <div class="mt-4">
                        <p class="text-sm text-gray-500 mb-2">Upload bukti transfer:</p>
                        <form action="{{ route('orders.upload', $order->id) }}" method="POST" enctype="multipart/form-data" class="flex items-center gap-3">
                            @csrf
